import { getAddress } from "viem";

export type TokenNetwork = "1" | "5" | "999" | "7777777" | "8453" | "84531";

export const TOKEN_NETWORK = (process.env.NEXT_PUBLIC_TOKEN_NETWORK ||
  "8453") as TokenNetwork;

export const TOKEN_CONTRACT =
  process.env.NEXT_PUBLIC_TOKEN_CONTRACT ||
  "0x220e41499cf4d93a3629a5509410cbf9e6e0b109";

export const MANAGER_CONTRACT = {
  "1": "0xd310A3041dFcF14Def5ccBc508668974b5da7174",
  "5": "0x0E9F3382Cf2508E3bc83248B5b4707FbA86D7Ee0",
  "999": "0xc521f85613985b7e417fccd5b348f64263d79397",
  "7777777": "0x3ac0e64fe2931f8e082c6bb29283540de9b5371c",
  "8453": "0x3ac0e64fe2931f8e082c6bb29283540de9b5371c",
  "84531": "0x550c326d688fD51ae65AC6A2d48749E631023A03",
}[TOKEN_NETWORK]!;

export const BASED_AND_YELLOW_MULTISIG = getAddress(
  "0x0b379F1848fA2aab7790966373618F4B0Fea6A6e"
);

export const COLLECTIVE_NOUNS_TREASURY = getAddress(
  "0x55333306a4c6e74eb9e23a521a24fb78be2de92c"
);

export const NOUNS_TOKEN_CONTRACT = "0x9C8fF314C9Bc7F6e59A9d9225Fb22946427eDC03";
